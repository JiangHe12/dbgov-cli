...
ORDER BY c.relname, a.attnum`, b.schema)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&tableName, &columnName, &columnType, &notNull, &defaultValue, &primary); err != nil {
			return schema.Schema{}, err
		}
		...
		column := schema.Column{Name: columnName, Type: columnType, Nullable: !notNull}
		if primary {
			column.Key = "PRI"
		}
...
func (b *Backend) RenderDDL(changes []schema.Change) ([]string, error) {
	...
	for _, change := range changes {
		...
		case schema.ActionAddColumn:
			statements = append(statements, fmt.Sprintf("ALTER TABLE %s ADD COLUMN %s %s;", quoteIdent(change.Table), quoteIdent(change.Column), change.Type))
		case schema.ActionModifyColumn:
			statements = append(statements, fmt.Sprintf("ALTER TABLE %s ALTER COLUMN %s TYPE %s;", quoteIdent(change.Table), quoteIdent(change.Column), change.Type))
